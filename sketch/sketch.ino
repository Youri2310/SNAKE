/* Ce qui à étét fait par l'ia en gros

  1. Les menus car jvais pas me casser le cul à dessiner hein et aussi une partie de la logique poru le retour au menu car j'avais la flemme de faire d'autre boucle if()
...
bool walls;
bool twoPlayers;
// Bool pour déterminer si je suis dans le menu ou pas, si oui le jeu ne tourne pas
bool inMenu;
// int qui permet l'affcihage de la selection du terrain ou du multi, en fonction de s'il est égale à 0 ou 1
int menuStage;
// garde en mémoire la reponse de menuStage = 0 en attendant qu'on selectionne le terrain avec ou sans murs
bool selMulti;
...
{
  if (menuStage == 0)
    drawMenu("Joueurs", "Solo", "Multi");
  else
    drawMenu("Terrain", "Sans murs", "Avec murs");
}
...
void applyKey(char c)
{
  ...
  else if (c == 's' && p1.dirY != -1)
  {
    p1.nextX = 0;
    p1.nextY = 1;
  }
  else if (c == 'q' && p1.dirX != 1)
  {
    p1.nextX = -1;
    p1.nextY = 0;
  }
  else if (c == 'd' && p1.dirX != -1)
  {
    p1.nextX = 1;
    p1.nextY = 0;
  }
  else if (twoPlayers && c == 'i' && p2.dirY != 1)
  {
    p2.nextX = 0;
    p2.nextY = -1;
  }
  else if (twoPlayers && c == 'k' && p2.dirY != -1)
  {
    p2.nextX = 0;
    p2.nextY = 1;
  }
  else if (twoPlayers && c == 'j' && p2.dirX != 1)
  {
    p2.nextX = -1;
    p2.nextY = 0;
  }
  else if (twoPlayers && c == 'l' && p2.dirX != -1)
  {
    p2.nextX = 1;
    p2.nextY = 0;
  }
